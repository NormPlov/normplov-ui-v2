...
export function DatePickerDemo({ selectedDate, onDateChange, disabled = false }: DatePickerDemoProps) {
  const [date, setDate] = useState<Date | undefined>(
    selectedDate && typeof selectedDate === "string" && isValid(parseISO(selectedDate))
      ? new Date(parseISO(selectedDate).toISOString())
      : undefined
  );
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (disabled) return; // Prevent input changes if disabled
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <div className="relative w-full">
          <input
            ...
            className={`w-full h-12 pl-4 mt-1 pr-12 text-left font-normal border ${
              disabled ? "bg-gray-100 cursor-not-allowed" : "border-slate-200 bg-white"
            } focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary text-textprimary placeholder-gray-400 text-md rounded-xl`}
      ...
      {!disabled && ( // Only show calendar if not disabled
